...
void SerRead()
{
    ...
    {  
        if(Serial.readStringUntil(',') == "M")//S
        {
        //Serial.println("seread");
        Motor = Serial.readStringUntil(','); // Motorwhl <1/0> 1 rechts 0 links
        pwm = Serial.parseInt(); // irgendwas zwischen 0 - 255
        richtung = Serial.parseInt(); // 0 vorwärts 1 rückwärts 
        }
        else if (Serial.readStringUntil(',') == "S") //S
        {
        ServoStr = Serial.readStringUntil(','); // Servowhl <1/0> 1 drehen 0 Kippen
